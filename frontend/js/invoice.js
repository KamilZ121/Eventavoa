...
$(document).ready(function () {
    ...
    $.ajax({type: "GET", url: ORDER_URL, data: {method: "getInvoice", order_id: orderId}, dataType: "json"})
        .done(showInvoice)
...
function showInvoice(response) {
    ...
    const totals = order.gutscheinbetrag > 0
        ? `<tr><th colspan="3" class="text-end">Zwischensumme</th><th class="text-end">${order.zwischensumme.toFixed(2)} €</th></tr>
           <tr class="text-success"><th colspan="3" class="text-end">Gutschein</th><th class="text-end">−${order.gutscheinbetrag.toFixed(2)} €</th></tr>
           <tr><th colspan="3" class="text-end">Zu zahlen</th><th class="text-end">${order.gesamt.toFixed(2)} €</th></tr>`
        : `<tr><th colspan="3" class="text-end">Gesamt</th><th class="text-end">${order.gesamt.toFixed(2)} €</th></tr>`;
    $("#invoice").html(`
        ...
        <table class="table"><thead><tr><th>Produkt</th><th class="text-end">Menge</th><th class="text-end">Einzelpreis</th><th class="text-end">Summe</th></tr></thead><tbody>${rows}</tbody><tfoot>${totals}</tfoot></table>
